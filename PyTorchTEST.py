...
parser.add_argument(
    "-bs", dest="batch_size", type=int, default=128, help="Batch Size (Default: 128)"
)
parser.add_argument(
    "-e",
    dest="epochs",
...
# ANR Hyperparameters
parser.add_argument(
    "-K", dest="num_aspects", type=int, default=5, help="Number of Aspects (Default: 5)"
)
parser.add_argument(
    "-h1",
    dest="h1",
...
parser.add_argument(
    ...
    type=str,
    default="",
    help="Specify the file name for loading pretrained ARL weights! (Default: "
    ", i.e. Disabled)",
)
parser.add_argument(
    "-ARL_lr",
...
parser.add_argument(
    "-gpu", dest="gpu", type=int, default=0, help="Which GPU to use? (Default: 0)"
)
parser.add_argument(
    "-vb",
    dest="verbose",
...
if args.use_cuda:
    ...
    print(
        "\n[args.use_cuda: {}] The program will be executed on the CPU!!".format(
            args.use_cuda
        )
    )
...
if args.save_model != "":
    ...
    model_path = "{}{}_{}.pth".format(
        saved_models_dir, args.save_model.strip(), args.random_seed
    )
...
train_set, train_loader, dev_set, dev_loader, test_set, test_loader = loadTrainDevTest(
    logger, args
)
logger.log(
    "Train/Dev/Test splits loaded! {}".format(
        timer.getElapsedTimeStr("init", conv2Mins=True)
    )
)
...
def evaluate(mdl, set_loader, epoch_num=-1, use_cuda=True, phase="Dev", print_txt=True):
    ...
    for batch_num, (batch_uid, batch_iid, batch_rating) in enumerate(set_loader):
        ...
        all_rating_true.extend(batch_rating.cpu().detach().numpy())
        all_rating_pred.extend(rating_pred.cpu().detach().numpy())

    MSE = mean_squared_error(all_rating_true, all_rating_pred)
    MAE = mean_absolute_error(all_rating_true, all_rating_pred)
    logger.log(
        "[{}] {:6s} MSE: {:.5f}, MAE: {:.5f}".format(
...
if not args.disable_initial_eval:
    ...
    logger.log(
        "\nInitial Evaluation Complete.. {}".format(
            timer.getElapsedTimeStr("init", conv2Mins=True)
        )
    )
...
opt = mdlZoo.selectOptimizer(
    optimizer=args.optimizer, learning_rate=args.learning_rate, L2_reg=args.L2_reg
)
logger.log(
...
for epoch_num in range(args.epochs):
    ...
    for batch_num, (batch_uid, batch_iid, batch_rating) in enumerate(
        ...
        rating_pred = torch.squeeze(
            mdl(
                batch_uid,
                batch_iid,
                verbose=(args.verbose > 0 and epoch_num == 0 and batch_num == 0),
            )
        )
    ...
    devMSE, _ = evaluate(
        mdl, dev_loader, epoch_num=epoch_num, use_cuda=args.use_cuda, phase="Dev"
    )
    testMSE, testMAE = evaluate(
        mdl, test_loader, epoch_num=epoch_num, use_cuda=args.use_cuda, phase="Test"
    )
    ...
    if args.save_model != "":
        ...
        if modelIsBest:
            ...
            logger.log(
                "\n*** MODEL has obtained the best DEV MSE of {:.5f} so far!".format(
                    devMSE
                )
            )
            ...
            filtered_mdl_state_dict = {
                k: v for k, v in filtered_mdl_state_dict.items() if k not in filter_list
            }
...
epoch_num_forBestDevMSE, bestDevMSE, testMSE_forBestDevMSE, testMAE_forBestDevMSE = (
    getBestPerf(lstDevMSE, lstTestMSE, lstTestMAE)
)
logger.log(
    "\nBest Dev MSE: {:.5f} (Obtained during Evaluation #{:d})".format(
        bestDevMSE, epoch_num_forBestDevMSE
    )
)
logger.log(
    "Test MSE: {:.5f}, Test MAE: {:.5f}".format(
        testMSE_forBestDevMSE, testMAE_forBestDevMSE
    )
)
